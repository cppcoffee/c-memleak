...
/// This crate has a runtime dependency on artifacts produced by the `c-memleak-ebpf` crate.
...
fn main() {
    ...
    let ebpf_package = packages
        .into_iter()
        .find(|Package { name, .. }| name == "c-memleak-ebpf")
        .unwrap();
    ...
    if build_ebpf {
        ...
        // We have a build-dependency on `c-memleak-ebpf`, so cargo will automatically rebuild us
        // if `c-memleak-ebpf`'s *library* target or any of its dependencies change. Since we
        // depend on `c-memleak-ebpf`'s *binary* targets, that only gets us half of the way. This
        ...
        let ebpf_target_dir = out_dir.join("c-memleak-ebpf");
